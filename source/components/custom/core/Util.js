const Util = {
    ...
    backtrace : function(node){
        const path = [[node.x, node.y, node.weight]]
        while (node.parent) {
            node = node.parent
    ...
    biBacktrace : function(nodeA, nodeB) {
        const pathA = this.backtrace(nodeA)
        const pathB = this.backtrace(nodeB)
     ...
     */
     expandPath : function(path) {
        ...
        let coord0, coord1
        let interpolated, interpolatedLen
        let i, j
        if (len < 2) {
            return expanded
        }
     ...
     */
     smoothenPath : function(grid, path) {
        ...
        let ex, ey                 // current end coordinate
        let newPath
        let i, j, coord, line, testCoord, blocked
        sx = x0
        sy = y0
        newPath = [[sx, sy]]
        for (i = 2; i < len; ++i) {
